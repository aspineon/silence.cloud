...
create table page (
  ...
  twittercard                   varchar(255) not null,
  twitterurl                    varchar(1000) not null,
  twittertitle                  varchar(71) not null,
  twitterdescription            varchar(201) not null,
  twitterimage                  varchar(1000) not null,
  constraint uq_page_page_name unique (page_name),
  ...
  constraint uq_page_twitterimage unique (twitterimage),
...
alter table page add constraint fk_page_status_id foreign key (status_id) references status (id) on delete restrict on update restrict;
create index ix_page_status_id on page (status_id);
...
alter table page drop constraint if exists fk_page_status_id;
drop index if exists ix_page_status_id;
